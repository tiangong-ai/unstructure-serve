...
| `docs/` | 当前专题文档；仅 AI 指南被路由显式读取 |
| `deploy/manage.sh` | 本项目组件的启动、停止、重启和状态入口 |
| `deploy/pm2/` | 稳定进程名、队列、退出窗口；cjs 根据配置文件位置解析项目目录 |
...
PM2 负责进程恢复，Gunicorn 管理 API 子进程，Docker 内 vLLM 管理模型副本。三者层次不同；增加 HTTP worker 不等于扩容模型。API 不 preload 已初始化的调度器/客户端。模型、Python 和依赖升级分别记录版本与恢复方法。

统一批量 CLI 进度日志写入 stderr，汇总 JSON 写入 stdout，文件日志由调用方重定向。兼容 two-stage 脚本默认日志才位于 output/logs，可用 TWO_STAGE_LOG_FILE 覆盖。批量任务的有界在途和 ID 持久化规则见[批量指南](batch-processing.md)。

## 批量客户端边界

`src/scripts/batch_parse.py` 负责 CLI 配置、文件扫描、HTTPX 流式上传及三个异步接口的字段适配；`batch_runner.py` 负责共享滚动窗口、原子落盘和任务恢复。`two_stage_enqueue.py` 为兼容入口，保持旧请求身份与 pickle 目录格式。统一客户端只通过公开 HTTP API，不直接调用 Celery 或读取服务端任务目录。
