...
应用与模型镜像的 MinerU 4.0.3 / 4.0.5 对照均使用三卡 DP3/TP1、vLLM 0.21.0、每卡显存比例 0.10，以及 `benchmark_mineru` 的三进程、VLM 并发 8、窗口 64、ONNX 16/1。预热后解析 p2、九页论文、46 页报告各两份（114 页），完成时间分别为 71.40 / 71.88 秒，整本页码、资产和 p2 关键表格校验通过。单轮差异约 0.7%，只能作为未见明显性能退化的检查，不能作为提速或长期吞吐承诺，也不包含 Celery 和独立图片识别。完整条件与结果保存在私有 `output/mineru405-upgrade/benchmark-before` 和 `benchmark-after`。
...
用固定六图或更大的业务图集，测试并发 1/3/6/12 等，每组记录成功率、每图及整批延迟、排队、429/超时、输出 token、图片模型 GPU 利用率和质量。不同端点单独测，再测组合；当前 vLLM 调度在本机共享端点轮换和并发槽位，`VLLM_VISION_ENDPOINT_SLOTS` 缺省每端点 16；所有调用方共用 `VLLM_VISION_SLOT_DIR`，缺省系统临时目录内 tiangong_vision_slots。临时连接/超时/408/429/5xx 及空/截断响应冷却 30 秒后进入单请求半开恢复，完整非空响应成功才恢复并发，400 等请求错误直接失败。等待槽位上限 180 秒；这些控制不跨主机，不按动态性能加权，也不保证文档级公平。改变槽数须排空调用方后统一切换共享目录，不能删除在用锁。

### 8.2 主动健康探测与恢复

`vision-health-monitor` 由 PM2 托管，app 组自动包含，也可单独管理 vision-health 组。它与 API/worker 使用相同私有配置及 `VLLM_VISION_SLOT_DIR`，同目录只有一个文件锁持有者执行探测；误启动的其他实例等待接管，进程退出自动释放。正常退出会取消在途探测。模型地址或凭证变更须共同重载探测进程与调用方。

| 参数 | 缺省 | 含义 |
| --- | ---: | --- |
| `VLLM_VISION_HEALTH_INTERVAL_SECONDS` | 10 秒 | 每轮起点间隔；一轮超时较长时不重叠启动 |
| `VLLM_VISION_HEALTH_TIMEOUT_SECONDS` | 2 秒 | 每端点健康与模型列表两次 GET 的总期限 |
| `VLLM_VISION_HEALTH_TTL_SECONDS` | 30 秒 | 状态有效期，须大于探测间隔；API/worker/探测进程保持一致 |
| `VLLM_VISION_COOLDOWN_SECONDS` | 30 秒 | 故障后等待一次半开推理的冷却时间 |

每轮最多并发探测 8 个端点，不执行图片生成。以配置的 `/v1` 为例，检查同前缀 `/health` 和 `/v1/models`，使用相同 Bearer 鉴权，不跟随重定向；仅健康接口 404/405 时回退模型列表。认证错误、5xx、超时或无有效模型列表均视为不可用。请求只选择模型列表包含目标模型的可用端点；全部端点被新鲜状态排除时直接失败，保留任务检查点。多个端点仍使用共同模型名，不做模型名称翻译或质量分级。

健康失败开启熔断，健康成功只允许冷却结束后试探；只有完整非空的真实推理响应成功才恢复并发。模型列表变化也需半开验证。恢复期间其他请求继续选择健康端点，无替代端点则等待共享准入预算；失败重新冷却。较旧请求成功不能清除新故障，进程死亡释放半开锁。探测进程停机后记录会过期，随后依赖业务请求作单请求恢复，不会因旧离线记录永久屏蔽端点；这段时间仍可能先遭遇一次连接/推理超时。

本地查看 `uv run python -m src.services.vision_health --status`，输出端点摘要、记录时间、fresh/healthy 和 circuit；recovery 表示需半开验证，不承诺已有请求在执行。`--once` 执行一轮探测并输出状态，仅在没有同目录常驻探测进程时使用。状态不经服务接口公开，不输出端点地址、凭证或模型名。健康成功不能保证实际推理或内容准确，真实图片回归仍必需。
...
vLLM 视觉客户端默认 `VLLM_VISION_CONNECT_TIMEOUT_SECONDS=5`、`VLLM_VISION_TIMEOUT_SECONDS=180`、`VLLM_VISION_MAX_RETRIES=0`。连接/TLS 使用独立的短预算，读写和连接池等待仍使用后者；若后者小于 5 秒，连接预算也随之缩短。连接失败、超时及临时服务错误后尝试其他端点，故障端点共享冷却 30 秒后仅放行一个半开推理请求，完整非空响应成功后恢复轮换；无需重启应用或删除该地址。全部端点失败仍使任务失败，400 等请求错误直接报告。这些是网络阶段预算，不是整份任务的墙钟上限；不能为加快连接故障切换而压缩正常推理时间。`VISION_LOG_PROMPTS=false` 默认不构造或记录完整上下文，排查私有数据问题时才显式打开。
...
同一构造混合 PDF 的持久普通任务以 advanced 完成 400 页纯解析，观测约 453.40 秒、6325 个业务块，全部页号、末页表格及源文件/结果摘要通过；2,675,759 B 结果独立下载，状态仅 179 B。并行的九页论文 two-stage 约 44.92 秒完成六张图片，此时长任务仍在解析。两个独立 CLI 均在短等待退出后按原 ID 续取，每文件一次提交、一次结果下载；再次运行跳过已有结果。本轮采用私有 API 1 / parse 2 / vision threads 32，wave=32，私有应用进程采样 PSS 峰值约 11.0 GiB，不含共享模型服务；同机还有其他验收负载，不能作为严格生产吞吐或提速对照。所有自建进程和队列已收尾；纯解析仍调用 MinerU 解析模型，不执行独立图片增强。

同一份 400 页混合样本的图片增强验收中，整本解析完成，108 个去重图片请求对应 598 个增强位置；但图片阶段发生两个端点各 180 秒超时，另一个技术成功的输出编造了 251 行递减气压值。因此这份带图任务没有通过质量验收。原 PDF 的相关图只有约 350×250 像素，缺少高清/矢量文字来源；不能用更长等待、返回 SUCCESS 或删除异常数值来证明识别正确。原始失败结果和已完成检查点均保留，提示词与采样候选须单独验证后采用。
