...
| 选项 | 新客户端缺省 | 含义 |
| --- | ---: | --- |
| `--max-in-flight` | 2 | 正在上传或已提交但尚未取回结果的文件数；不代表 GPU 数或图片请求数 |
| `--upload-concurrency` | 2 | 独立上传线程上限，受在途窗口约束 |
| `--query-concurrency` | 4 | 独立状态/结果查询线程上限，不与上传互相阻塞 |
...
上传与查询使用独立的有界线程池，慢上传不会阻挡已完成任务的收取，慢查询也不会挡住有空位时的下一次上传。记录与结果由主线程统一落盘。提交前创建匿名文件快照，并对照启动扫描时的 SHA256；上传过程中源文件被修改也不会改变已发送内容。快照占用临时磁盘，退出自动释放。网络故障退出时会等待已经发送的上传返回，保存能够取回的任务 ID，因此退出可能受上传超时影响。

客户端先查询轻量 `/tasks/{id}/status`，仅 SUCCESS 后下载 `/tasks/{id}/result`，服务端无需在状态轮询中加载、序列化全文；客户端仍会解析并保存完整业务 JSON。只有状态接口返回 404 时才兼容回退旧模式 GET；认证错误、网络错误和其他 HTTP 错误不触发回退，EXPIRED 明确停止并保留本地记录。输出继续保持普通模式省略 null、two-stage 保留 null，已有批次可直接续跑。
...
已记录 FAILED 的任务也先查询原 ID：运维修复原因并执行服务端 resume 后，原命令或 `--resume-only` 可以收取同一 ID 的新结果。客户端不会自动调用服务端 resume；仅在查询仍明确失败、未启用 resume-only 且尝试预算允许时，才创建下一次尝试。

计划停机时，停止原 CLI 后用**相同命令加 `--resume-only`**重新运行，只收取已知任务结果，不提交未开始的文件，也不重试失败文件；汇总中的 `deferred` 是尚未提交的文件数。必须使用已有批次目录；服务恢复后去掉该选项即可继续整批。先等结果收取及服务端队列收敛，再停止 API/worker/模型。
...
- POST 响应丢失、无法确认 task_id：保留 `SUBMITTING`，停止新增提交。先由运维核查服务端是否接受并找回任务 ID；不能删除记录后盲目重投。新 CLI 已保存原 Idempotency-Key；确认原始内容/参数一致后可据此找回同一任务，方法见 [AI 指南 §5.4](ai-integration.md#54-提交幂等轻量查询与阶段恢复)。SUBMITTING 不自动重传。
- POST 返回 `503` 且包含合法 `detail.task_id` / `publication=uncertain`：保存已知 ID 后停止新增提交；续跑查询该 ID。若消息尚未发布，由运维执行发布恢复，不重新上传文件。
- 旧版任务的 `PENDING` 也可能是结果过期或未知 ID，不应自动视为失败重投。及时保存每份成功结果；不要等整批结束后才下载。
