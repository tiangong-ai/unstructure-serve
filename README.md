...
language: zh-CN
whenToUse: "When initializing, recovering, stopping or using this service."
...
在能访问部署机器的 AI 中打开本仓库作为工作区，再复制以下任一段。AI 会读取本地配置使用凭证，无需把密钥粘贴到对话中。整套设施包括本项目的 API、普通 worker、六个 two-stage worker、视觉端点探测进程和三卡 MinerU 模型，以及配置中依赖的 Redis、图片模型；共享或其他项目管理的依赖须先识别归属。
...
通过 deploy/manage.sh start model 启动模型，等待实际健康；再执行 start app 和 start ordinary，启动 API、六个 two-stage worker、视觉端点探测进程与普通 worker。已有在线进程不要重复重启。
...
核对 API、三个解析 worker、视觉/调度/合并 worker、普通 worker、视觉端点探测进程和三卡模型，并做小规模真实 PDF 验收。检查批量输出目录中的任务记录及持久任务清单（uv run python -m src.scripts.manage_jobs list），发布不明确的任务按部署说明 recover；失败任务修复原因并确认没有活跃阶段后才按原 ID resume。恢复客户端时使用原命令、原输出目录续查，不重复提交已有 task_id。
...
任务收敛后，依次执行 deploy/manage.sh stop api、stop ordinary、stop workers、stop vision-health、stop model，确认本项目进程已停止、MinerU 容器已退出，再 pm2 save，使停止状态在重启后保持。
...
| 视觉端点探测进程 | 主动探测独立图片端点，向 API/worker 共享状态 | 图片增强；start app 自动包含 |
...
`app` 包含 API、六个 two-stage worker 和视觉端点探测进程；`ordinary` 启动普通 worker。以下脚本命令均从仓库根目录执行；重复 start 会跳过已在线进程，修改配置请使用 restart。PM2 模板位于 `deploy/pm2`，模型 YAML 位于 `deploy/mineru-vllm`，Gunicorn 参数位于 `deploy/gunicorn.conf.py`。可选 Flower 不会自动启动。
