...
在能访问部署机器的 AI 中打开本仓库作为工作区，再复制以下任一段。AI 会读取本地配置使用凭证，无需把密钥粘贴到对话中。整套设施包括本项目的 API、普通 worker、two-stage worker、视觉端点探测进程和按硬件选择的三卡或四卡 MinerU 模型，以及配置中依赖的 Redis、图片模型；共享或其他项目管理的依赖须先识别归属。
...
| 程序 | 用途 | 何时需要 |
| --- | --- | --- |
| Docker MinerU VLM | 解析模型；三卡 DP=3 或四卡 DP=4，单地址 30000 | standard/advanced |
| Gunicorn API | 上传、提交、查询，默认 7770 | 所有服务调用 |
| Redis | Celery 队列与结果状态 | 异步任务 |
| 普通 Celery worker | 纯解析或完整图片增强任务 | 两个普通 `/task` 接口 |
| two-stage worker | 三卡组六个（其中解析三个），四卡组七个（其中解析四个） | 图片增强批处理 |
...
- 三卡模板要求 GPU 0/1/2 可用，四卡模板要求 GPU 0/1/2/3 可用；按[部署说明](docs/mineru_4_upgrade_usage.md#docker-与多卡)选择对应组，并按目标显卡调整显存预算。
